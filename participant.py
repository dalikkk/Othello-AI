...
class MyPlayer:
    ...
    def move(self, board):
        ...
        # you can implement auxiliary fucntions, of course eg
        #
        my_board = MyGameBoard(board)
        # you can simulate moves here so you do not have to reinvent the wheel
        #my_board.play((1, 1))
        # this does not affect gameplay
        # obtain modified board as
        #my_board.board or
        #my_board.get_board_copy()

        moves = self.get_all_valid_moves(board)
        return moves[random.randint(0, 100000) % len(moves)]
